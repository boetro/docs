...
We're excited to anounce the addition of the [endpoint](/programming-guide/endpoints) pattern which enables serving HTTP traffic via an API. With endpoints you can easily standup a scalable backend API with a few lines of code.
...
In addition to endpoints we're also adding the [collector](/programming-guide/collectors) pattern. Which enables receiving HTTP requests and dumping them to the sink of your choice.
...
We've added support for [GCP Cloud SQL](/primitives/gcp/cloud_sql) instance and database primitives. These primitives allow you to easily manage resources, and can be used with our new [dependencies](#dependencies) feature to easily connect to your database.
...
In just a few lines of code you have a config management system for your database and a dependency that can be injected into any processor to maintain a [consistant database connection](/dependencies/sqlalchemy).