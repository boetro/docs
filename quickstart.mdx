...
Once installed you will now have access to the `buildflow` CLI tool. First create a directory for your project and then run the `init` command which will prompt you to enter some basic information about your project.
...
<Tip>
We recommend starting with an endpoint since it requires the least setup. You can always add more processors later.
</Tip>

When you run `buildflow init` you'll see the following prompts (feel free to name your project whatever your like):

```
$ buildflow init
Please enter a project name: hello
Please enter a processor type (endpoint, consumer, collector): endpoint
```
