...
Creating a deployment is as simple as clicking the `Launch Deployment` button in the VSCode extension, or by using the `launch deploy` command in the CLI. Once your deployment is launched it will be assigned a unique ID that you can use to manage it.
