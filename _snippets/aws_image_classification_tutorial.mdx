...
In this tutorial we'll introduce you to BuildFlow and BuildFlow concepts by walking through a simple image classification application.
...
The `buildflow.yaml` defines our [buildflow project](/buildflow/programming-guide/buildflow-yaml). This is created when we use the [BuildFlow CLI](/buildflow/reference/cli/init) to create a new project, and doesn't need to be updated unlesss you need to add something.
...
This file is where we start to define our logic. `app = Flow(...)` creates our [Flow object](/buildflow/programming-guide/flows) which is a container for all of our buisness logic and resources.
...
Here we define all of the [primitives](/buildflow/programming-guide/primitives) used by our applications. Primitives are resources that a processor may read from, write to, or referenced.

<Tip>
    You may have noticed these same primitives were passed into `app.manage` this means they will be created when you run `buildflow run` for the first time.
</Tip>
...
In `dependencies.py` we set up a [dependencies](/buildflow/programming-guide/dependencies) that will be used by our endpoints and consumer. Dependencies allow us to precompute / share code between processors. In our case we want to load the model when we start our application before any data is processed.

We do this by defining the `ModelDep` you can see we add the decorator `@dependency(scope=Scope.REPLICA)` which annotates the class as a dependency that should be created before a [replica](/buildflow/concepts#replicas) starts to process data.
...
## processors/service.py

In `service.py` we define our actual endpoints that will be used by our users.  First we create a service with `service = Service(...)` then attach endpoints to functions using `@service.endpoint(...)`.
...
- **image_upload**: this endpoint allows users to upload an image and we upload them to our remote bucket. It takes in an [UploadFile](/buildflow/programming-guide/requests#uploadfile) which is a file that has been uploaded by a user. We also take in the `bucket_dep` dependency we defined earlier. This allows us to upload the file to our bucket using the [boto3 API](https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/s3/bucket/put_object.html).
