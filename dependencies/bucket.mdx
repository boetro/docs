...
The dependency will provide and object containing a reference to the bucket in the `bucket` field.
...
@app.endpoint("/", method="POST")
def endpoint(bucket_dep: BucketDep):
    objects = bucket_deb.bucket.objects.all()
```
