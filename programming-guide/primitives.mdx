...
Primitives can be uses as sources or sinks in your [processors](/programming-guide/processors), and they can also be managed by your [flow](/programming-guide/flows).
If a primitive is managed it can automatically be created, managed, and destroyed with BuildFlow

BuildFlow provides many primitives out of the box, and you can also create your own custom primitives. All of our primitives can be found in our [primitive docs](/primitives/)
...
Both of these commands will print out a preview of the resources that will be created or destroyed. If you are happy with the plan you can run type: `yes` to accept it.

If you would like to simply see a preview you can run `buildflow plan`
...
To create a custom primitivey you can implement the [Primitive](/reference/api/primitive) interface a description of all the methods is found below (bold indicates required).


<Tip>
If your primitive is specific to a cloud provider you can extend [GCPPrimitive](/reference/api/primitive#gcp-primitive) or [AWSPrimitive](/reference/api/primitive#aws-primitive).
...
| source                | [SourceStrategy](/reference/api/strategy#source-strategy) | Returns an implementation of [SourceStrategy](/reference/api/strategy#source-strategy) for reading data.
| sink                  | [SinkStrategy](/reference/api/strategy#sink-strategy)   | Returns an implementation of [SinkStrategy](/reference/api/strategy#sink-strategy) for reading data.                       |                       |
