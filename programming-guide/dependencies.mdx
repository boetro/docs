...
BuildFlow allows you to define custom dependencies for your processor that can be injects whenever your processor is called. This enable you to easily integrate other components into your processor.
...
<Tip>
    Even though we only use endpoints in the above example, dependencies and be used with any processor pattern (endpoints, consumers, or collectors),
</Tip>
...
In our above exapmle we mark the dependency as `GLOBAL` which means it will be created when the flow is started and destroyed when the flow is drained. This means that the dependency will be available for the lifetime of the flow and will be shared across all processors. So no matter how many times you call your processor it will always reference the same string object.
...
You'll also note that we marked our `ConnectDep` dependency as `REPLICA` scoped so this database connection will be kept alive and reused for all calls to your processor (unless a new replica as added for a scaling event).
