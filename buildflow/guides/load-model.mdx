...
In this guide we will go over how to load and serve a model that is stored in a remote bucket (GCS or S3). We will load the model at server startup and ensure that it gets used across requets.
...
- [A custom dependency](/buildflow/programming-guide/dependencies) to load the model from a remove bucket
