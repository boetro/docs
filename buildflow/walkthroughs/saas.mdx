...
In this walkthrough we will be creating a simple CRUD API that allows users to log in with their Google account and create and update journal entries. These entries will be stored in a Postgres database hosted on GCP and will be accessible to the user when they log in.

Our API will be built using [endpoints](/programming-guide/endpoints) and our databased will be hosted on [Google Cloud SQL](http://localhost:3000/buildflow/primitives/gcp/cloud_sql) that we connect to via [SQLAlchemy](http://localhost:3000/buildflow/dependencies/sqlalchemy).
...
To run this walkthrough on resources created on GCP use [this github repo](https://github.com/launchflow/gcp-saas-example).
<Note>    
