...
The [buildflow.yaml](/programming-guide/buildflow-yaml) file is a configuration file that defines your application. This can be used to configure what primitives and resources your application manages and connects to.


## Flows

The [Flow](/programming-guide/flows) class is the entrypoint into the BuildFlow Framework. They acts as a container type for user-defined Processors and are responsible for orchestrating the Processors across the Runtime and Infrastructure submodules.
...
At a high-level, [rocessors](/programming-guide/processors) are a user-defined function that are individually scalable. 
...
### Endpoints
An [endpoint](/programming-guide/endpoints) is a processor pattern intended for serving data over HTTP.
...
A [collector](/programming-guide/collectors) is a processor pattern intended for receiving data over HTTP and dumping it to a sink.
...
### Consumers
A [consumer](/programming-guide/consumers) is a processor pattern intended for event driven data processing. Consumers read from an unbounded source such as (Kafka, AWS SQS, or GCP Pub/Sub) and outputs to a Sink. They are attached to a Flow by using the `@app.consumer` decorator.
...
A service is a group of endpoints that are deployed and scaled together. Services are defined by call `app.service()` decorator.
...
[Primitives](/programming-guide/primitives) represent an individual resource that a processor may read, write to, or simply reference. For example, a `BigQueryTable` is a primitive that represents a single BigQuery table. Primitives define how to read from a source, write to a sink, and also how to manage the primitive with pulumi. 

Primitives can be used as inputs (Sources) or outputs (Sinks) for consumers and collectors. A full list of our support primitives can be found in our [primitive docs](/primitives/). You can also [define your own primitives](/programming-guide/primitives#creating-custom-primitives) for reading, writing, and managing custom resources.

## Dependencies

[Dependencies](/programming-guide/dependencies) are any thing that is needed to call your processor. For example maybe you need to open a connection to your database, or maybe you need to download a model from a remote bucket.
